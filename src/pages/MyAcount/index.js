...
const MyAcount = () => {
  ...
  const getToken = () => {
    ...
    if (token) {
      setDecode(
        jwt_decode(JSON.parse(localStorage.getItem("@GestãoDeHábitos:access")))
      );
    }
  };
  ...
  const handleSubmitFunction = (data) => {
    ...
    api
      ...
      .then(() => {
        if (token) {
          api
            .patch(`/users/${decode.user_id}/`, submit, {
  ...
  function removei() {
    return new Promise(() => {
      localStorage.removeItem("@GestãoDeHábitos:access");
      getTokenAtt();
    });
  }
    ...
    <Container>
      <AnimationContainer>
        <form onSubmit={handleSubmit(handleSubmitFunction)}>
          ...
          <Button size={"40%"} margin={"40px"} onClick={() => handleLogout()}>
            Logout
          </Button>
        </form>
      </AnimationContainer>
